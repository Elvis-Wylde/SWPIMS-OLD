...
public class JsonTextParser {
	private static String[] jsKeywords = { "abstract", "boolean", "break", "byte", "case", "catch", "char", "class",
			"const", "continue", "debugger", "default", "delete", "do", "double", "else", "enum", "export", "extends",
			"fasle", "final", "finally", "float", "for", "function", "goto", "if", "implements", "import", "in",
	...
	public Json parse() throws IOException, JsonException {
		...
		while (ch != -1) {
			if (ch == '{') {
				...
				String msg = "Cannot found json object begin sign '{'" + " or json array begin sign ']' at position "
						+ pos + ".";
	...
	/**
	 * 从当前字符开始解析JsonObject实例， 进入时pos指向字符'{'，退出时指向对应的'}'之后的第一个字符。
	 * 
		...
		while (ch != -1) {
			...
			if (isBlankCharacter(ch)) {
				...
				if (json.containsName(name)) {
					String msg = "Object element name \"" + name + "\" at position " + pos + " is repeated.";
					throw new JsonException(msg);
				} else {
	...
	/**
	 * 从当前字符开始解析JsonArray实例， 进入时pos指向字符'['，退出时指向对应的']'之后的第一个字符。
	 * 
	...
	/**
	 * 解析JsonObject子元素的name部分，进入时指向name部分（可包括前导空白）的第一个字符， 退出时指向字符':'后的第一个字符。
	 * 
	...
	private String parseName() throws IOException, JsonException {
		...
		while (ch != -1) {
			if (ch == '\'' || ch == '\"') {
				...
				// 无引号字符串
				if (ch == '_' || ch == '$' || (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch > 256) {
					str = parseString();
					parseTailBlank(':');
					break;
				} else {
	...
	/**
	 * 解析Json子元素（对JsonObject而言指子元素的Value部分）， 进入时指向表示value（可包含前导空白）的第一个字符， 退出时指向value（不包含尾空白）之后的第一个字符。
	 * 
	...
	/**
	 * 解析不带引号的字符串（用于处理JsonObject子元素的Name部分）， 进入时pos指向字符串的开头，退出时指向字符串的下一个字符。
	 * 
	 * @return 所解析的字符串
	 * @throws IOException 读取Reader发生异常
	...
	private String parseString() throws IOException, JsonException {
		...
		while (ch != -1) {
			...
			if (ch > 256 || ch == '_' || ch == '$' || (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z')
					|| (ch >= '0' && ch <= '9')) {
	...
	private String parseString(int quoteChar) throws IOException, JsonException {
					...
					case 'u':
						for (int i = 0; i < 4; i++) {
							...
							if ((ch >= '0' && ch <= '9') || (ch >= 'a' && ch <= 'f') || (ch >= 'A' && ch <= 'F')) {
								...
								String msg = "Un expected character found in unicode character sequence at positon "
										+ pos + ".";
								throw new JsonException(msg);
							}
		...
		if (ch == quoteChar) {
			...
			msg += (quoteChar == '"') ? "\'" + (char) quoteChar + "\'" : "\"" + (char) quoteChar + "\"";
			msg += " at position " + pos + ".";
			throw new JsonException(msg);
		}
	 ...
	 */
	private JsonPrimitive parseJsonConstant(String constName, int[] constAry, int endChar) throws IOException,
			JsonException {
		...
		json = (constAry == trueAry) ? Json.trueJson : (constAry == falseAry) ? Json.falseJson : Json.nullJson;

		return json;
	}

	/**
	 * 从当前字符开始解析剩余的空白字符串，遇到终止字符前出现其他字符或EOF为异常。 进入时pos指向空白字符，或endsChar中的字符， 退出时指向遇到的第一个endsChar字符。
	 * 
	...
	private void parseTailBlank(int endChar1, int endChar2) throws IOException, JsonException {
		...
		while (ch != -1) {
			if (ch == endChar1 || ch == endChar2) {
				...
				String msg = "Non-blank  character found at position " + pos + " before '" + (char) endChar1
						+ "' and '" + (char) endChar2 + "'." + " character is \"" + ch + "\"";
				throw new JsonException(msg);
			}
		...
		if (!found) {
			String msg = "Cannot found end character \'" + (char) endChar1 + "\' and \'" + (char) endChar2
					+ "\' at position " + pos + ".";
			throw new JsonException(msg);
		}
	}
	...
	private void parseTailBlank(int endChar) throws IOException, JsonException {
		...
		while (ch != -1) {
			if (ch == endChar) {
				...
				msg += (endChar == -1) ? "EOF." : "'" + (char) endChar + "'." + " character is \"" + ch + "\"";
				throw new JsonException(msg);
			}
	...
	/**
	 * 将字符串尽量以不带引号的形式追加到字符流末尾，如果无法转换为不带引号的字符串， 则以带引号的字符串输出。
	 * 
	 * @param str 要追加的字符串
	 * @param dest 接受字符串的字符流对象
	 * @throws IOException 发生IO异常
	...
	protected static void jsonStringToAppendableWithoutQutoe(String str, Appendable dest) throws IOException {
		...
		if (len == 0) {
			...
			for (int i = 0; i < len; i++) {
				...
				if (!(c > 256 || c == '_' || c == '$' || (c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'))) {
					canToNoquote = false;
					break;
				}
			}
