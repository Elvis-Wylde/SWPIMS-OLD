...
/**
 * <p>
 * 所有Json实例的抽象超类，用来提供操作Json的通用接口。
 * </p>
 * <p>
 * <strong>生成Json实例</strong>，把文本转换为Json实例可以调用类方法
 * <strong>parseJsonText</strong>或<strong>parseJsonReader</strong>，
 * 如果想从Java集合生成Json实例，则可以调用类方法<strong>parseJavaMap</strong> 或<strong>parseJavaCollection</strong>。
 * </p>
 * <p>
 * <strong>生成Json文本</strong>，调用方法<strong>generateJsonText</strong>
 ...
 * </p>
 * <p>
 * 方法<strong>isEmpty</strong>可以判断Json实例子元素的个数是否为零， 方法<strong>count</strong>返回子元素的的个数，
 * 方法<strong>clear</strong>可以清除所有的子元素。 方法<strong>getType</strong>返回Json实例的类型。
 * </p>
 * <p>
 * 创建、修改Json实例的过程中可能会产生Json实例间的<strong>循环引用</strong>， 可以用方法<strong>existsCircle</strong>检测。
 * </p>
 * 
 * @author 飞翔的河马
 * @version 1.01
...
public abstract class Json implements Cloneable {
	 ...
	 * </p>
	 * <p>
	 * Map的Key只允许是String、Number与Boolean，其他类型的Key会导致异常。
	 * 如果Map的Value是String、Number或者Boolean，则转换为相应的JsonPrimitive实例；
	 * 如果是Jsonable实例，将调用方法generateJson转换；如果是Collection对象， 则调用parseJavaCollection(Collection)转换；如果是Map对象，
	 * 则递用parseJavaMap(Map)转换；如果Value是除String、Number、Boolean、Jsonable、 Json、Collection与Map之外的类型，则会导致异常。
	 * </p>
	 * <p>
	 * Value部分的空指针会转换为NULL类型的Json实例，Jsonable实例的generateJson 方法若返回空指针，也会转换为NULL类型的Json实例。
	 * </p>
	 * 
	 ...
	 * </p>
	 * <p>
	 * 如果Map的Key能通过JsonParser的方法canToName，则调用JsonParser的方法
	 ...
	 * 如果是String、Number或者Boolean，则转换为相应的JsonPrimitive实例； 如果是Jsonable实例，将调用方法generateJson转换；如果是Collection对象，
	 * 则调用parseJavaCollection(Collection, JsonParser)转换；如果是Map对象， 则递用parseJavaMap(Map,
	 * JsonParser)转换；如果Value是除String、Number、 Boolean、Jsonable、Json、Collection与Map之外的类型，则会导致异常。
	 * </p>
	 * <p>
	 * Value部分的空指针会转换为NULL类型的Json实例，Jsonable实例的generateJson 方法若返回空指针，也会转换为NULL类型的Json实例。
	 * </p>
	 ...
	 * @since 1.01
	 */
	public static JsonObject parseJavaMap(Map<?, ?> map, JsonParser parser) throws JsonException {
		map.remove(null);
	 ...
	 * </p>
	 * <p>
	 * 如果Collection的子元素是String、Number或者Boolean，则转换为相应的JsonPrimitive实例；
	 * 如果是Jsonable实例，将调用方法generateJson转换；如果是Collection对象，
	 * 则调用parseJavaCollection(Collection)转换；如果是Map对象，则递用parseJavaMap(Map)转换；
	 * 如果Value是除String、Number、Boolean、Jsonable、Json、Collection与Map之外的类型， 则会导致异常。
	 * </p>
	 * <p>
	 * 空指针会转换为NULL类型的Json实例，Jsonable实例的generateJson方法若返回空指针， 也会转换为NULL类型的Json实例。
	 * </p>
	 * 
	 ...
	 * </p>
	 * <p>
	 * 如果Collection的子元素能通过JsonParser的方法canToJson，则调用JsonParser
	 * 的方法changeToJson转换为Json实例。对于JsonParser的方法canToJson不接受的子元素，
	 * 如果是String、Number或者Boolean，则转换为相应的JsonPrimitive实例； 如果是Jsonable实例，将调用方法generateJson转换；如果是Collection对象，
	 * 则调用parseJavaCollection(Collection, JsonParser)转换；如果是Map对象， 则递用parseJavaMap(Map,
	 * JsonParser)转换；如果子元素是除String、Number、 Boolean、Jsonable、Json、Collection与Map之外的类型，则会导致异常。
	 * </p>
	 * <p>
	 * 空指针会转换为NULL类型的Json实例，Jsonable实例的generateJson方法若返回空指针， 也会转换为NULL类型的Json实例。
	 * </p>
	 ...
	 * @since 1.01
	 */
	public static JsonArray parseJavaCollection(Collection<?> collection, JsonParser parser) throws JsonException {
		IdentityStack parentRef = new IdentityStack();
	 ...
	 */
	protected static Json changeToJson(Object value, JsonParser parser) throws JsonException {
		Json json = null;
	...
	/**
	 * 判断是否相等，如果都是Json实例，类型相同，子元素的个数相同， 且对应的子元素相等（对JsonArray来讲，下标相同的子元素相等，
	...
	/**
	 * 深层Clone一个Json实例，Clone出来的新实例与原实例相等（euqals()返回true）， 但修改任何一个实例都不会影响另一个实例的值。
	 * 
	...
	/**
	 * Json实例的类型。</br> <strong>OBJECT</strong>表示Json对象，用类型JsonArray存储；</br>
	 * <strong>ARRAY</strong>表示Json数组，用类型JsonArray存储；</br> <strong>STRING</strong>表示Json字符串，用类型String存储；</br>
	 * <strong>INTEGER</strong>表示Json整型数值，用类型Long存储；</br> <strong>FLOAT</strong>表示Json浮点型数值，用类型Double存储；</br>
	 * <strong>BOOLEAN</strong>表示Json逻辑型，用类型Boolean存储；</br> <strong>NULL</strong>表示Json null类型，用类型String存储。
	 * 
	 * @author 飞翔的河马
	 * @version 1.00
	 */
