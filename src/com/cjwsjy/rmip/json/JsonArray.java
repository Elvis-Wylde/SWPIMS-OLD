...
/**
 * <p>
 * 用来表示Json数组实例。Json数组是一组有序Json实例的集合， 数组的每个子元素都有特定的<strong>下标（从0开始）</strong>， 可以根据下标存取特定的子元素。
 * </p>
 * <p>
 * <strong>创建JsonArray实例</strong>， 除了可以通过调用Json类的parse开头的方法创建JsonArray实例外，
 * 还可以直接创建空的JsonArray实例，或从Json实例集合创建包含子元素的JsonO实例。
 * </p>
 * <p>
 * 调用方法<strong>get</strong>可以获取指定下标的子元素，方法<strong>getXXX
 * </strong>以特定的类型返回指定下标子元素的值，方法<strong>canToXXX</strong> 判定指定下标的子元素是否可以转换为这种类型，重设指定下标的子元素调用方法
 ...
 * </p>
 * <p>
 * JsonArray实现了接口<strong>Iterable</strong>，可以用<strong>for each</strong>
 * 循环遍历所有的子元素，也可以用方法<strong>iterator</strong>获取JsonArray的迭代器。
 * </p>
 * <p>
 * 方法<strong>isEmpty</strong>可以判断JsonArray实例子元素的个数是否为零， 方法<strong>count</strong>返回子元素的的个数，
 * 方法<strong>clear</strong>可以清除所有的子元素。 方法<strong>getType</strong>
 * 返回JsonArray实例的类型<strong>JsonType.ARRAY</strong>。
 * </p>
 * 
 * @author 飞翔的河马
 * @version 1.00
...
public final class JsonArray extends Json implements Iterable<Json> {
	...
	public boolean append(Json element) {
		return (element == null) ? this.elements.add(Json.nullJson) : this.elements.add(element);
	}
	...
	public boolean append(String element) {
		return (element == null) ? this.elements.add(Json.nullJson) : this.elements.add(new JsonPrimitive(element));
	}
	...
	public void insert(int index, Json element) {
		if (element == null) this.elements.add(index, Json.nullJson);
		else this.elements.add(index, element);
	}

	/**
	 * 在指定下标向Json数组添加一个新子元素，原来的子元素依次后移， 相当于insert(index, element.generateJson())。
	 * 
	...
	public void insert(int index, Jsonable element) {
		Json json = (element == null) ? Json.nullJson : element.generateJson();
		if (json == null) this.elements.add(index, Json.nullJson);
		else this.elements.add(index, json);
	}
	...
	public void insert(int index, String element) {
		if (element == null) this.elements.add(index, Json.nullJson);
		else this.elements.add(index, new JsonPrimitive(element));
	}
	...
	public void set(int index, Json element) {
		if (element == null) this.elements.set(index, Json.nullJson);
		else this.elements.set(index, element);
	}
	...
	public void set(int index, Jsonable element) {
		Json json = (element == null) ? Json.nullJson : element.generateJson();
		if (json == null) this.elements.set(index, Json.nullJson);
		else this.elements.set(index, json);
	}
	...
	public void set(int index, String element) {
		if (element == null) this.elements.set(index, Json.nullJson);
		else this.elements.set(index, new JsonPrimitive(element));
	}
	...
	/**
	 * JsonArray实例的hash值，根据子元素及其下标计算hashCode， 不同的排列顺序（对应的JsonArray也不等）生成hashCode不同。
	 * 
	 * @return 对应的hashCode值
	 */
	@Override
	...
	/**
	 * 深层Clone一个JsonArray实例，Clone出来的新实例与原实例相等（euqals()返回true）， 但修改任何一个实例都不会影响另一个实例的值。
	 * 
