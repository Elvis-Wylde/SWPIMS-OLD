...
/**
 * <p>
 * 用来表示Json对象实例。Json对象是由子元素（即Name Value对）的无序集合构成， 可以通过Name存取对应子元素的Value。
 * </p>
 * <p>
 * JsonObject不存在也不允许存在Name为null的子元素。<br/>
 * JsonObject不存在也不允许存在Value为null的子元素， 如果添加的新子元素的Value为null，或者将子元素的Value改为null， 则会自动转换为NULL类型的Json实例。
 * </p>
 * <p>
 * <strong>创建JsonObject实例</strong>， 除了可以通过调用Json类的parse开头的方法创建JsonObject实例外，
 * 还可以直接创建空的JsonObject实例，或从Json实例集合创建包含子元素的JsonObject实例。
 * </p>
 * <p>
 * 调用方法<strong>get</strong>可以获取指定Name的子元素，方法<strong>getXXX
 * </strong>以特定的类型返回指定Name的子元素的Value值，方法<strong>canToXXX</strong>
 * 判定指定Name的子元素是否可以转换为这种类型，重设指定Name的子元素的Value调用方法 <strong>set</strong>，批量重设调用方法<strong>setAll</strong>，
 * 添加子元素调用方法<strong>add</strong>，批量添加调用方法<strong>addAll</strong>， 删除指定Name的子元素调用方法<strong>remove</strong>。
 * （方法<strong>add与set的区别</strong>是如果存在指定Name的子元素，方法add抛出异常，
 * 方法set覆盖子元素原来的Value；如果Name为null，方法add抛出异常，但set忽略这次操作）
 * </p>
 * <p>
 * 获取所有子元素的Name组成的集合调用方法<strong>nameSet</strong>， 获取所有子元素的Value组成的集合调用方法<strong>values</strong>，
 * 获取所有子元素组成的集合调用方法<strong>entrySet</strong>。
 * </p>
 * <p>
 * 方法<strong>isEmpty</strong>可以判断JsonObject实例子元素的个数是否为零， 方法<strong>count</strong>返回子元素的的个数，
 * 方法<strong>clear</strong>可以清除所有的子元素。 方法<strong>getType</strong>返回JsonObject实例的类型JsonType.OBJECT。
 * </p>
 * 
 * @author 飞翔的河马
 * @version 1.00
...
public final class JsonObject extends Json {
	...
	public Json get(String name) {
		return ((name == null) || (!this.elements.containsKey(name))) ? null : this.elements.get(name);
	}
	...
	public boolean canToJsonArray(String name) {
		return ((name != null) && (this.elements.containsKey(name))) && this.elements.get(name) instanceof JsonArray;
	}
	...
	public boolean canToJsonObject(String name) {
		return ((name != null) && (this.elements.containsKey(name))) && this.elements.get(name) instanceof JsonObject;
	}
	...
	public void add(String name, Json value) throws JsonException {
		...
		if (value == null) this.elements.put(name, Json.nullJson);
		else this.elements.put(name, value);
	}
	...
	public void add(String name, Jsonable value) throws JsonException {
		...
		if (value == null) {
			...
			if (json == null) this.elements.put(name, Json.nullJson);
			else this.elements.put(name, json);
		}
	}
	...
	public void add(String name, String value) throws JsonException {
		...
		if (value == null) this.elements.put(name, Json.nullJson);
		else this.elements.put(name, new JsonPrimitive(value));
	}
	 ...
	 * @throws JsonException 如果Map内含有循环引用或无法解析的对象， 或试图加入同名子元素， 或加入Name为null的子元素，则抛出异常
	 */
	public void addAll(Map<?, ?> map) throws JsonException {
		if (map.containsKey(null)) throw new JsonException("Try to add element with name is null.");
	...
	public void addAll(JsonObject jobj) throws JsonException {
		...
		for (String name : names) {
			if (this.elements.containsKey(name)) conflictNames += ", " + name;
		}

		if (conflictNames.equals("")) {
			this.elements.putAll(jobj.elements);
		} else {
			String msg = "Try to add exists names \"" + conflictNames.substring(2) + "\" to this JsonObject.";
			throw new JsonException(msg);
		}
	}
	...
	public Json set(String name, Json value) {
		...
		return (value == null) ? this.elements.put(name, Json.nullJson) : this.elements.put(name, value);
	}
	...
	public Json set(String name, Jsonable value) {
		...
		return (json == null) ? this.elements.put(name, Json.nullJson) : this.elements.put(name, json);
	}
	...
	public Json set(String name, String value) {
		if (name == null) return null;
		return (value == null) ? this.elements.put(name, Json.nullJson) : this.elements.put(name, new JsonPrimitive(
				value));
	}
	...
	public Json set(String name, long value) {
		return (name == null) ? null : this.elements.put(name, new JsonPrimitive(value));
	}
	...
	public Json set(String name, boolean value) {
		return (name == null) ? null : this.elements.put(name, Json.getBooleanJson(value));
	}
	...
	public Json set(String name) {
		return (name == null) ? null : this.elements.put(name, Json.nullJson);
	}

	/**
	 * 批量设置子元素，值为null的value当做类型为NULL的Json实例处理， 如果存在同名Name则覆盖原来的Value。
	 * 
	...
	/**
	 * 批量添加子元素， 子元素来自另外一个JsonObject实例， 如果存在同名Name则覆盖原来的Value。
	 * 
	...
	/**
	 * 深层Clone一个JsonObject实例，Clone出来的新实例与原实例相等（euqals()返回true）， 但修改任何一个实例都不会影响另一个实例的值。
	 * 
	 ...
	 */
	@Override
	// if modify this method, modify appendToWriter() together
	...
	public JsonType getType(String name) {
		if (name == null || (!this.elements.containsKey(name))) return null;
		else return this.elements.get(name).getType();
	}
