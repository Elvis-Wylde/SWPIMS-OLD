...
/**
 * <p>
 * 用来表示STRING、NUMBER（LONG与DOUBLE）、BOOLEAN与NULL类型的Json实例， 创建后不可更改，且无需显式调用构造函数创建。
 * </p>
 * <p>
 * 方法<strong>getXXX</strong>以特定的类型返回Json实例的值， 方法<strong>canToXXX</strong>判定Json实例值是否可以转换为这种类型。
 * 方法<strong>getType</strong>返回Json实例的类型， 方法<strong>isEmpty、count、clear</strong>仅仅是从超类Json继承来的占位符， 无实际意义。
 * </p>
 * 
 * @author 飞翔的河马
 * @version 1.00
...
public final class JsonPrimitive extends Json {
	...
	/**
	 * 判断Json实例是否可以转换为逻辑型（布尔型）值，如果类型是BOOLEAN， 或是类型是STRING且值为true或false（不区分大小写）， 则认为可以转换。
	 * 
	...
	/**
	 * 判断Json实例是否可以转换为整型值，如果类型是INTEGER， 或类型是STRING且其值表示一个整数，则认为可以转换。
	 * 
	 * @return 是返回true，否则返回false
	 */
	...
	/**
	 * 判断Json实例是否可以转换为浮点型值，如果类型是INTEGER或FLOAT， 或者是STRING且其值表示一个数值，则认为可以转换。
	 * 
	 * @return 是返回true，否则返回false
	 */
	public boolean canToDouble() {
		if (this.type == JsonType.FLOAT || this.type == JsonType.INTEGER) {
			...
			String str = this.data.toString();
			return str.matches("\\s*[+-]?\\d+(\\.\\d*)?([eE][+-]?\\d+)?\\s*");
		} else {
			return false;
		}
	}
